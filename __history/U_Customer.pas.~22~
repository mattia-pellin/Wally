...
type
  TF_Customer = class(TForm)
    ...
    LE_contact_mail: TLabeledEdit;
    DTP_deadline: TDateTimePicker;
    Label1: TLabel;
    procedure FieldCheckUncommonValue(Sender: TObject);
...
begin
  Database.AddCustomer(LE_name.Text, LE_address.Text, LE_address_city.Text, LE_address_province.Text,
                       LE_contact_phone.Text, LE_contact_fax.Text, LE_contact_mail.Text, LE_tax_code.Text,
...
begin
  ...
  begin
    ...
    if ( not(Database.FieldValueExists(Field, (Sender as TLabeledEdit).Text)) ) then
...
procedure TF_Customer.FieldCheckUncommonValue(Sender: TObject);
var
  Field: String;
  Empty: Boolean;
begin
